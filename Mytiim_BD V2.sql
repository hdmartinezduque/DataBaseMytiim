...
create table if not exists t_objective(
	...
	objective_qualify int null default 0,
...
-- drop view v_objective
ALTER TABLE t_objective ALTER COLUMN objective_describe TYPE varchar(600);
...
* Hernan M
* 2023-05-09
* 1. Se crea tabla de tipo o categoria de comentarios
* 2. Se agrega columna comment_comment_type_id a la tabla t_comment
* 3. Se agrega regla relacional a la tabla t_comment - fk_comment_comment_type_id con el campo creado y la tabla creada
*/

alter table t_status add column status_param int default 0;
alter table t_module add column module_param int default 0;

drop table if exists t_comment_type;
create table if not exists t_comment_type(
	comment_type_id serial4 not null,
	comment_type_describe varchar(255) not null,
	comment_type_status_id int not null,
	constraint pk_id_comment_type PRIMARY KEY (comment_type_id)
);

alter table t_comment add column comment_comment_type_id int default 0;

alter table t_comment 
add constraint fk_comment_comment_type_id foreign key(comment_comment_type_id) 
references t_comment_type(comment_type_id);

insert into t_comment_type(comment_type_describe, comment_type_status_id) values('Avance',2);
insert into t_comment_type(comment_type_describe, comment_type_status_id) values('Reconocimiento', 2);
insert into t_comment_type(comment_type_describe, comment_type_status_id) values('Retroalimentacion', 2);


/*
* Hernan M
* 2023-05-09
* 1. Agragar columna parametrica a la tabla t_status status_param
* 2. Se definen parametros para el status de acuerdo al modulo - los cuales serán relacionados en la tabla t_module en la columna module_param
* 3. se agrega el parametro
*/

update t_status set status_param = 1 where t_status.status_id <6;
insert into t_status(status_describe, status_param) values('Activo', 2);

/*
* Saul Echeverri
* 2023-05-10
* 1. Se agregan los atributos de avance y meta en la tabla compromisos (t_commitment)
* 2. Se adiciona la relación entre las tablas compromisos y objetivos
*/
...
create table if not exists t_commitment(
	commitment_id serial4 not null,
	commitment_objective_id int not null,
	commitment_measure_id int not null,
	commitment_describe varchar(255) not null,
	commitment_date date not null,
	commitment_advance varchar(100) not null,
	commitment_goal varchar(100) not null,
	constraint pk_commitment_id PRIMARY KEY (commitment_id)
);
...
* Saul Echeverri
* 2023-05-11
* 1. Se crea tabla de tipo o categoria de medidas (t_measure)
* 2. Se modifica el atributo measure_describe 
* 3. Se adiciona la relación entre las tablas medidas y compromiso
*/

drop table if exists t_measure ;
create table if not exists t_measure(
	measure_id serial4 not null,
	measure_describe int null default 0 CHECK(measure_describe >= 0),
	constraint pk_measure_id PRIMARY KEY (measure_id)
);

alter table t_commitment add constraint fk_commitment_measure_id foreign key(commitment_measure_id) references t_measure(measure_id);
...
* Saul Echeverri
* 2023-05-11
* 1. Se modifica el tipo de dato del atributo measure_describe
*/

ALTER TABLE t_measure drop COLUMN measure_describe;
ALTER TABLE t_measure ADD COLUMN measure_describe varchar(10);
...
* Alexander Alfaro
* 2023-05-15
* se agrega campo para el username a usar en login
* se modifica tabla de status
*/
ALTER TABLE t_user ADD COLUMN user_user varchar(35);
...
update t_user set user_user='adminpruebas', user_password = '123' where user_email ='adminpruebas@cidenet.com.co';

ALTER TABLE t_status ADD COLUMN status_type varchar(1);
comment on column t_status.status_type is 'U=user, O=objective';

ALTER TABLE t_status ALTER COLUMN status_id DROP DEFAULT;
update t_status set status_type = 'O' where status_id > 0;

insert into t_status(status_id,status_describe,status_type) values(1,'ACTIVO','O');
insert into t_status(status_id,status_describe,status_type) values(2,'ATRASADO','O');
insert into t_status(status_id,status_describe,status_type) values(3,'EN RIESGO','O');
insert into t_status(status_id,status_describe,status_type) values(4,'EN PAUSA','O');
insert into t_status(status_id,status_describe,status_type) values(5,'CERRADO','O');
insert into t_status(status_id,status_describe,status_type) values(6,'ACTIVO','U');
insert into t_status(status_id,status_describe,status_type) values(7,'ACTIVACION PENDIENTE','U');
...
* Alexander Alfaro
* 2023-05-19
* se cambia el  tipo de dato en la BD
*/
update t_commitment set commitment_advance = 10.,commitment_goal=100;
ALTER TABLE t_commitment ALTER COLUMN commitment_advance TYPE float8 USING commitment_advance::float8;
ALTER TABLE t_commitment ALTER COLUMN commitment_goal TYPE float8 USING commitment_advance::float8;
